...
tf.app.flags.DEFINE_string('output_graph', '/home/niusilong/work/AI/graph/competition_inception_v4_25000.pb', """Where to save the trained graph.""")
tf.app.flags.DEFINE_integer('print_count', 10, """""")
tf.app.flags.DEFINE_float('min_score', 0.12, """最低预测分数""")
...
print("output_graph: ",FLAGS.output_graph)
    ...
    with tf.Session() as sess:
        ...
        for step in range(total_steps):
            ...
            for i in range(len(final_tensor_val)):
                ...
                for j in range(len(sorted_array)):
                    ...
                    if final_tensor_val[i][sorted_array[j]] < FLAGS.min_score or j >= FLAGS.max_predict_count:
                ...
                predict_output_array.append(image_paths_val[i].split("/")[-1]+" "+re.sub("['\\[\\]\\s]","",str(prediction_output_result)))
        ...
        for i in range(len(files)):
            if files[i].startswith(FLAGS.predict_output_file_name):
                count += 1
        export_file_path = os.path.join(FLAGS.predict_output_file_dir, FLAGS.predict_output_file_name+"_"+str(count)+".txt")
