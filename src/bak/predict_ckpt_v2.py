...
tf.app.flags.DEFINE_string('predict_output_file', "/home/niusilong/work/AI/predict/predict_result.txt", """最低预测分数""")
    ...
    with tf.Session() as sess:
        ...
        for step in range(total_steps):
            ...
            for i in range(len(final_tensor_val)):
                ...
                prediction_output_result = [str(label_index+1) for label_index in sorted_array[:prediction_end_index]]
                prefix = ""    #调整得分的缩进空格
        ...
        # print("------------------------------------------------")
        for i in range(len(all_image_paths)):
            print(changed_predict_array[i])
        with tf.gfile.GFile(FLAGS.predict_output_file, "wb") as f:
            for i in range(len(all_image_paths)):
                f.write(predict_output_array[i]+"\n")
