...
tf.app.flags.DEFINE_float('to_add_score', 0.35, """调整得分数字""")
...
def main(argv=None):
    ...
    with tf.Session() as sess:
        ...
        for step in range(total_steps):
            ...
            for i in range(len(final_tensor_val)):
                sorted_array = final_tensor_val[i].argsort()[::-1]
                final_tensor_val[i][sorted_array[0]] += 0.5
                final_tensor_val[i][sorted_array[1]] += 0.5
                final_tensor_val[i][sorted_array[2]] += 0.45
                final_tensor_val[i][sorted_array[3]] += 0.39
                final_tensor_val[i][sorted_array[4]] += 0.25
                # final_tensor_val[i][sorted_array[5]] += 0.2
                prediction_result=[LABEL_DICT[label_index+1]+"("+str(label_index+1)+"):"+str(final_tensor_val[i][label_index]) for label_index in sorted_array[:FLAGS.print_count]]
                prefix = ""    #调整得分的缩进空格
                for j in range(len(image_paths_val[i])-13):
                    prefix += " "
