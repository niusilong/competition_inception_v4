...
tf.app.flags.DEFINE_string('src_image_dir', "/home/niusilong/projects/image-cut/src/main/webapp/images/120_3", """""")
tf.app.flags.DEFINE_string('crop4_image_dir', "/home/niusilong/projects/image-cut/src/main/webapp/images/crop4_120_3_128", """四等分图片目录""")
    ...
    with tf.Session() as sess:
        ...
        for _ in range(FLAGS.output_file_count):   #导出文件的个数
            ...
            for step in range(total_steps):
                ...
                # sigmoid_val = tf.nn.sigmoid(final_tensor_val).eval()
                ...
                for i in range(len(final_tensor_val)):
                    ...
                    prediction_result_mean=[LABEL_DICT[label_index+1]+"("+str(label_index+1)+"):"+str(sigmoid_val[i][label_index]) for label_index in sorted_array[:FLAGS.print_count]]
                ...
                for i in range(len(final_tensor_val)):
                    sorted_array = sigmoid_val[i].argsort()[::-1]
                    ...
                    for j in range(len(sorted_array)):
                        if j >= FLAGS.predict_label_count:
                            ...
                            break
                    prediction_result=[LABEL_DICT[label_index+1]+"("+str(label_index+1)+"):"+str(sigmoid_val[i][label_index]) for label_index in sorted_array[:prediction_end_index]]
                    prediction_output_result = [str(label_index+1)+"("+str(sigmoid_val[i][label_index])+")" for label_index in sorted_array[:prediction_end_index]]
