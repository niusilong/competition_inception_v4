...
PRINT_SCORE = True
tf.app.flags.DEFINE_string('image_dir', "/home/niusilong/svn/svn_repository_ai/image-cut/src/main/webapp/images/120_3", """""")
...
tf.app.flags.DEFINE_string('graph_file', '/home/niusilong/work/AI/graph/competition-inception_v4_4740_niusilong_bbox_25500.pb', """Where to save the trained graph.""")

tf.app.flags.DEFINE_integer('print_count', 10, """""")
tf.app.flags.DEFINE_float('min_score', 0.13, """最低预测分数, 平均数约为0.0038461538""")
tf.app.flags.DEFINE_integer('min_predict_count', 6, """最低标签数量""")
tf.app.flags.DEFINE_integer('max_predict_count', 6, """最高标签数量""")
tf.app.flags.DEFINE_float('exceed_min_score', 0.5, """超出数量限制后依然的效的最低分`""")
        ...
        for _ in range(FLAGS.output_file_count):   #导出文件的个数
            ...
            for step in range(total_steps):
                ...
                for i in range(len(final_tensor_val)):
                    ...
                    predict_mean = np.mean(sigmoid_val[i], axis=0)
                    prediction_result_mean=[LABEL_DICT[label_index+1]+"("+str(label_index+1)+"):"+str(sigmoid_val[i][label_index]) for label_index in sorted_index_array[:FLAGS.print_count]]
                ...
                for i in range(len(final_tensor_val)):
                    sorted_index_array = sigmoid_val[i].argsort()[::-1]
                    prediction_end_index = len(sorted_index_array)-1
                    for j in range(len(sorted_index_array)):
                        if j < FLAGS.min_predict_count:
                            continue
                        elif j >= FLAGS.max_predict_count:
                            if sigmoid_val[i][sorted_index_array[j]] >= FLAGS.exceed_min_score:
                                continue
                            else:
                                prediction_end_index = j
                                break
                        else:
                            if sigmoid_val[i][sorted_index_array[j]] < FLAGS.min_score:
                                prediction_end_index = j
                                break
                    prediction_result=[LABEL_DICT[label_index+1]+"("+str(label_index+1)+"):"+str(sigmoid_val[i][label_index]) for label_index in sorted_index_array[:prediction_end_index]]
                    prediction_output_result = [str(label_index+1)+(("("+str(sigmoid_val[i][label_index])+")") if PRINT_SCORE else "") for label_index in sorted_index_array[:prediction_end_index]]
