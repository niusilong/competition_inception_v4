...
tf.app.flags.DEFINE_float('to_add_score', 0.3, """调整得分数字""")
...
def main(argv=None):
    ...
    add_num = tf.constant(FLAGS.to_add_score, shape=final_tensor.get_shape().as_list(), dtype=tf.float32)
    final_tensor_changed_score = tf.add(final_tensor, add_num)
    #计算正确率
    ...
    with tf.Session() as sess:
        ...
        for step in range(total_steps):
            ...
            for i in range(len(final_tensor_val)):
                ...
                predict_array.append(image_paths_val[i]+" "+str(prediction_result)+"\n"+prefix+str(final_tensor_changed_score_val_result))
            accuracy_array.append("step:%d, Accuracy: %.2f%%" % (step+1, 100.0*sess.run(evaluation_step, feed_dict={images:images_val, multi_hot_labels: multi_hot_labels_val})))
