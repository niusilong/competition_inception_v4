...
tf.app.flags.DEFINE_string('image_dir', "/home/niusilong/work/AI/competition_test/1", """""")
...
tf.app.flags.DEFINE_float('min_score', 0.065, """最低预测分数""")
...
tf.app.flags.DEFINE_integer('output_file_count', 50, """最低预测分数""")
    ...
    with tf.Session() as sess:
        ...
        for _ in range(FLAGS.output_file_count):   #导出文件的个数
            ...
            for step in range(total_steps):
                ...
                percent_val = get_percentage(final_tensor_val)
                # print("percent_val:",percent_val)
                percent_val = np.reshape(percent_val, newshape=[FLAGS.batch_size, 260])
                # print("np.shape(final_tensor_val):", np.shape(final_tensor_val), ", np.shape(percent_val):",np.shape(percent_val))
                for i in range(len(final_tensor_val)):
                    ...
                    predict_mean = np.mean(percent_val[i], axis=0)
                    prediction_result_mean=[LABEL_DICT[label_index+1]+"("+str(label_index+1)+"):"+str(percent_val[i][label_index]) for label_index in sorted_array[:FLAGS.print_count]]
                ...
                for i in range(len(final_tensor_val)):
                    sorted_array = percent_val[i].argsort()[::-1]
                    prediction_end_index = len(sorted_array)-1
                    for j in range(len(sorted_array)):
                        if j < FLAGS.min_predict_count: continue
                        if percent_val[i][sorted_array[j]] < FLAGS.min_score or j >= FLAGS.max_predict_count:
                            prediction_end_index = j
                            break
                    prediction_result=[LABEL_DICT[label_index+1]+"("+str(label_index+1)+"):"+str(percent_val[i][label_index]) for label_index in sorted_array[:prediction_end_index]]
