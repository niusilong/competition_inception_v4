...
with tf.Session() as sess:
    ...
    with tf.variable_scope("add_num"):
        add_num = tf.get_variable("bias", shape=[5], initializer=tf.truncated_normal_initializer(stddev=0.1), trainable=True)
        add_num_row = tf.concat([add_num, tf.constant(0, shape=[255], dtype=tf.float32)], axis=0)
        add_num_stack_array = []
        for i in range(FLAGS.batch_size):
            add_num_stack_array.append(add_num_row)
        add_num_batch = tf.stack(add_num_stack_array)
    tf.global_variables_initializer().run()
    tf.local_variables_initializer().run()
    predict_array = []
    accuracy_array = []
    for step in range(total_steps):
        print("add_num.eval():",add_num.eval())
        ...
        final_tensor_changed = tf.add(final_tensor, add_num_batch)
        print("tf.trainable_variables():",tf.trainable_variables())
        with tf.name_scope('evaluation_changed'):
            correct_prediction = tf.equal(tf.round(final_tensor_changed), tf.constant(np.asarray(multi_hot_labels), dtype=tf.float32))
            evaluation_step = tf.reduce_mean(tf.cast(correct_prediction, tf.float32), name='accuracy_changed')

        loss = evaluation_step #tf.constant(1, dtype=tf.float32, shape=[1])-evaluation_step
        train_step = tf.train.RMSPropOptimizer(0.0001).minimize(loss, global_step=global_step)

        sess.run(train_step, feed_dict={FLAGS.image_batch_name+":0": images, FLAGS.image_batch_name+":2": multi_hot_labels})
