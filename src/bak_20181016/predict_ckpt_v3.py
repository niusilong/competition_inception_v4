#coding=utf-8
'''打乱次序后再预测, 并导出预测文件， 设置shuffle参数来指定是否改变图片次序'''
import glob
import os.path
import numpy as np
...
tf.app.flags.DEFINE_float('min_score', 0.1, """最低预测分数""")
    ...
    with tf.Session() as sess:
        ...
        for step in range(total_steps):
            ...
            for i in range(len(final_tensor_val)):
                ...
                # predict_output_array.append(image_paths_val[i]+" "+re.sub("['\\[\\]\\s]","",str(prediction_output_result)))
