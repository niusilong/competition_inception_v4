...
with tf.Session() as sess:
    ...
    for step in range(total_steps):
        ...
        print("len(image_paths_val):",len(image_paths_val))
        for i in range(len(final_tensor_val)):
            ...
            for j in range(len(sorted_array)):
                if final_tensor_val[i][sorted_array[j]] < FLAGS.min_score:
                    ...
                    break
            predict_ids = [str(j+1) for j in sorted_array[:prediction_end_index]]
            prediction_result=[LABEL_DICT[j+1]+"("+str(j+1)+"):"+str(final_tensor_val[i][j]) for j in sorted_array[:prediction_end_index]]
            predict_array.append(image_paths_val[i]+" "+str(predict_ids)+" "+str(prediction_result))
