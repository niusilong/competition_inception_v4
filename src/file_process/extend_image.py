...
import numpy as np
import random
IMAGE_LABEL_FILE="/home/niusilong/svn/svn_repository_ai/competition_inception_v4/label_file/4740_absolute_path.txt"
OUTPUT_LABEL_FILE = "/home/niusilong/svn/svn_repository_ai/competition_inception_v4/label_file/4740_absolute_path_gt30.txt"
...
def extend_images():
    ...
    original_image_lines = get_image_labels()
    changed_images = []
    for i in range(len(original_image_lines)):
        changed_images.append(original_image_lines[i].line)
    print(changed_images)
    total_added_count = 0
    for i in range(len(label_count)):
        if label_count[i] >= LABEL_MIN_COUNT:
            continue
        label = ALL_LABELS[i]
        count = label_count[i]
        added_count = 0
        # for j in range(len(original_image_lines)):
        original_image_index = 0
        while True:
            if label in original_image_lines[original_image_index].labels:
                changed_images.append(original_image_lines[original_image_index].line)
                added_count += 1
                if count + added_count >= LABEL_MIN_COUNT:
                    break
            if original_image_index == len(original_image_lines)-1:
                original_image_index = 0
            else:
                original_image_index += 1
        print("label:%d, count:%d, added_count:%d" % (label, count, added_count))
        total_added_count += added_count
    print("len(changed_images):",len(changed_images))
    print("total_added_count:",total_added_count)
    print(changed_images)
    print_label_count(get_label_count(changed_images))
    return changed_images
if __name__ == '__main__':
    extended_images = extend_images()
    # random.shuffle(extended_images)
    # print(extended_images)
    with open(OUTPUT_LABEL_FILE, "w") as f:
        for line in extended_images:
            line_array = line.split(" ")
            if APPEND_PATH:
                f.write(IMAGE_PATH_PREFIX+line_array[0]+IMAGE_SUFFIX+" "+(line_array[1] if len(line_array) == 2 else "")+"\n")
            else:
                f.write(line_array[0]+" "+(line_array[1] if len(line_array) == 2 else "")+"\n")