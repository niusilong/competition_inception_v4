...
from file_process.strategy import convert_score
...
PRINT_SCORE = True
SCORE_TYPE = 'sigmoid'   #sigmoid, softmax
CONVERT_TO_STANDARD_SCORE = False
tf.app.flags.DEFINE_string('image_dir', "/home/niusilong/svn/svn_repository_ai/image-cut/src/main/webapp/images/120_4", """""")
tf.app.flags.DEFINE_integer('image_size', 192, """""")
tf.app.flags.DEFINE_integer('batch_size', 30, """""")
tf.app.flags.DEFINE_string('graph_file', '/home/niusilong/work/AI/graph/competition-inception_v4_4860_resetted_appointed_24000.pb', """Where to save the trained graph.""")

# tf.app.flags.DEFINE_string('image_dir', "/home/niusilong/svn/svn_repository_ai/image-cut/src/main/webapp/images/crop4_120_4_128", """""")
...
# tf.app.flags.DEFINE_string('graph_file', '/home/niusilong/work/AI/graph/competition-inception_v4_crop4_4740_115000.pb', """Where to save the trained graph.""")
...
# tf.app.flags.DEFINE_string('graph_file', '/home/niusilong/work/AI/graph/competition_inception_v4_25000.pb', """Where to save the trained graph.""")
# tf.app.flags.DEFINE_string('graph_file', '/home/niusilong/work/AI/graph/competition_inception_v4_extended_19000.pb', """Where to save the trained graph.""")
# tf.app.flags.DEFINE_string('graph_file', '/media/niusilong/芥子空间/multi-label-competition-inception_v4_192_18000.pb', """Where to save the trained graph.""")

tf.app.flags.DEFINE_integer('print_count', 10, """""")
tf.app.flags.DEFINE_integer('min_predict_count', 7, """最低标签数量""")
tf.app.flags.DEFINE_integer('max_predict_count', 7, """最高标签数量""")
tf.app.flags.DEFINE_string('predict_output_file_dir', "/home/niusilong/work/AI/predict", """最低预测分数""")
tf.app.flags.DEFINE_string('predict_output_file_name', ("score_full_predict_result" if PRINT_SCORE else "full_predict_result"), """""")
tf.app.flags.DEFINE_integer('output_file_count', 100, """导出文件个数""")
...
print("shuffle:",FLAGS.shuffle,", min_predict_count:", FLAGS.min_predict_count,", max_predict_count:", FLAGS.max_predict_count)
...
def main(argv=None):
    with tf.Session() as sess:
        ...
        for _ in range(FLAGS.output_file_count):   #导出文件的个数
            init_data()
            ...
            for step in range(total_steps):
                ...
                if SCORE_TYPE == 'sigmoid':
                    sigmoid_val = final_tensor_val
                    # print("sigmoid_val:",sigmoid_val)
                    sigmoid_val = np.reshape(sigmoid_val, newshape=[FLAGS.batch_size, 260])
                    # print("np.shape(final_tensor_val):", np.shape(final_tensor_val), ", np.shape(sigmoid_val):",np.shape(sigmoid_val))
                    final_score_val = sigmoid_val
                elif SCORE_TYPE == 'softmax':
                    softmax_val = tf.nn.softmax(final_tensor_val).eval()
                    # print("softmax_val:",softmax_val)
                    softmax_val = np.reshape(softmax_val, newshape=[FLAGS.batch_size, 260])
                    final_score_val = softmax_val
                for i in range(len(final_tensor_val)):
                    sorted_index_array = final_tensor_val[i].argsort()[::-1]
                    prediction_result=[LABEL_DICT[label_index+1]+"("+str(label_index+1)+"):"+str(final_tensor_val[i][label_index]) for label_index in sorted_index_array[:FLAGS.print_count]]
                    if CONVERT_TO_STANDARD_SCORE:
                        convert_score.convert_to_standard_score(final_score_val[i], 1/final_score_val[i][sorted_index_array[0]])
                    # print("final_score_val[i][0:10]: ",final_score_val[i][0:10])
                    predict_mean = np.mean(final_score_val[i], axis=0)
                    # print("predict_mean:",predict_mean)
                    prediction_result_mean=[LABEL_DICT[label_index+1]+"("+str(label_index+1)+"):"+str(final_score_val[i][label_index]) for label_index in sorted_index_array[:FLAGS.print_count]]
                ...
                for i in range(len(final_tensor_val)):
                    sorted_index_array = final_score_val[i].argsort()[::-1]
                    prediction_end_index = len(sorted_index_array)-1
                    for j in range(len(sorted_index_array)):
                        if j >= FLAGS.max_predict_count:
                            prediction_end_index = j
                            break
                    prediction_result=[LABEL_DICT[label_index+1]+"("+str(label_index+1)+"):"+str(final_score_val[i][label_index]) for label_index in sorted_index_array[:prediction_end_index]]
                    prediction_output_result = [str(label_index+1)+(("("+str(final_score_val[i][label_index])+")") if PRINT_SCORE else "") for label_index in sorted_index_array[:prediction_end_index]]
