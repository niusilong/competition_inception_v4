...
import os
import re
from file_process import merge_crop4_predict_file

# os.environ["CUDA_VISIBLE_DEVICES"]="-1"
# tf.app.flags.DEFINE_string('image_dir', "/home/niusilong/work/AI/images_test", """""")
SCORE_TYPE = 'softmax'   #sigmoid, softmax
tf.app.flags.DEFINE_string('src_image_dir', "/home/niusilong/svn/svn_repository_ai/image-cut/src/main/webapp/images/120_3", """""")
tf.app.flags.DEFINE_string('crop4_image_dir', "/home/niusilong/svn/svn_repository_ai/image-cut/src/main/webapp/images/crop4_120_3_128", """四等分图片目录""")
...
tf.app.flags.DEFINE_string('graph_file', '/home/niusilong/work/AI/graph/competition-inception_v4_crop4_4740_103000.pb', """Where to save the trained graph.""")
# tf.app.flags.DEFINE_string('graph_file', '/home/niusilong/work/AI/graph/competition-inception_v4_4500_cropped4.pb', """Where to save the trained graph.""")

tf.app.flags.DEFINE_integer('print_count', 10, """""")
tf.app.flags.DEFINE_integer('predict_label_count', 7, """""")
tf.app.flags.DEFINE_string('predict_output_file_dir', "/home/niusilong/work/AI/predict", """最低预测分数""")
tf.app.flags.DEFINE_string('predict_output_file_name', "crop4_predict_result", """""")
tf.app.flags.DEFINE_integer('output_file_count', 50, """导出文件个数""")
...
print("shuffle:",FLAGS.shuffle, ", predict_label_count:", FLAGS.predict_label_count)
...
def init_data():
    ...
    all_image_paths = []
    array_index_array = []
    image_list = os.listdir(FLAGS.crop4_image_dir)
    image_list.sort()
    for image_name in image_list:
        if image_name.split(".")[-1] in EXTENSIONS:
            all_image_paths.append(os.path.join(FLAGS.crop4_image_dir, image_name))
...
def sort_img(filepath):
    try:
        filename = filepath.split("/")[-1].split(".")[0]
        if "_" in filename:
            return int(filename.replace("_", ""))
        else:
            return int(filename)
    except:
        print("exception sort img!")
        return filename
...
def main(argv=None):
    with tf.Session() as sess:
        ...
        for _ in range(FLAGS.output_file_count):   #导出文件的个数
            ...
            for step in range(total_steps):
                ...
                for i in range(len(final_tensor_val)):
                    ...
                    prediction_result_mean=[LABEL_DICT[label_index+1]+"("+str(label_index+1)+"):"+str(final_score_val[i][label_index]) for label_index in sorted_array[:FLAGS.print_count]]
                ...
                for i in range(len(final_tensor_val)):
                    sorted_array = final_score_val[i].argsort()[::-1]
                    prediction_end_index = len(sorted_array)-1
                    for j in range(len(sorted_array)):
                        if j >= FLAGS.predict_label_count:
                            prediction_end_index = j
                            break
                    prediction_result=[LABEL_DICT[label_index+1]+"("+str(label_index+1)+"):"+str(final_score_val[i][label_index]) for label_index in sorted_array[:prediction_end_index]]
                    prediction_output_result = [str(label_index+1)+"("+str(final_score_val[i][label_index])+")" for label_index in sorted_array[:prediction_end_index]]
        ...
        #合并预测后的文件
        merge_crop4_predict_file.write_merged_files(FLAGS.src_image_dir, FLAGS.predict_output_file_dir, FLAGS.output_file_count)
